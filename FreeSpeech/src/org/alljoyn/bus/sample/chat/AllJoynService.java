...
public class AllJoynService extends Service implements Observer {
	...
	/**
	 * Our onCreate() method is called by the Android appliation framework when
	 * the service is first created. We spin up a background thread to handle
	 * any long-lived requests (pretty much all AllJoyn calls that involve
	 * communication with remote processes) that need to be done and insinuate
	 * ourselves into the list of observers of the model so we can get event
	 * notifications.
	 */
	@Override
	 ...
	 * startService(). We expect that the only place this is going to be done is
	 * when the Android Application class for our application is created. The
	 * Appliation class provides our model in the sense of the MVC application
	 * we really are.
	 * 
	 ...
	 * our Application class is left running as long as the process is left
	 * running.
	 */
	@Override
	 ...
	 * 
	 * This method will be called in the context of the Model, which is, in turn
	 * the context of an event source. This will either be the single Android
	 * application framework thread if the source is one of the Activities of
	 * the application or the Service. It could also be in the context of the
	 * Service background thread. Since the Android Application framework is a
	 * fundamentally single threaded thing, we avoid multithread issues and
	 * deadlocks by immediately getting this event into a separate execution in
	 * the context of the Service message pump.
	 * 
	 * We do this by taking the event from the calling component and queueing it
	 * onto a "handler" in our Service and returning to the caller. When the
	 * calling componenet finishes what ever caused the event notification, we
	 * expect the Android application framework to notice our pending message
	 * and run our handler in the context of the single application thread.
	 * 
	 * In reality, both events are executed in the context of the single Android
	 * thread.
	 */
	@Override
	...
	/**
	 * This is the Android Service message handler. It runs in the context of
	 * the main Android Service thread, which is also shared with Activities
	 * since Android is a fundamentally single-threaded system.
	 * 
	 * The important thing for us is to note that this thread cannot be blocked
	 * for a significant amount of time or we risk the dreaded "force close"
	 * message. We can run relatively short-lived operations here, but we need
	 * to run our distributed system calls in a background thread.
	 * 
	...
	/**
	 * Value for the HANDLE_APPLICATION_QUIT_EVENT case observer notification
	 * handler.
	 */
	private static final int HANDLE_APPLICATION_QUIT_EVENT = 0;

	/**
	 * Value for the HANDLE_USE_JOIN_CHANNEL_EVENT case observer notification
	 * handler.
	 */
	private static final int HANDLE_USE_JOIN_CHANNEL_EVENT = 1;

	/**
	 * Value for the HANDLE_USE_LEAVE_CHANNEL_EVENT case observer notification
	 * handler.
	 */
	private static final int HANDLE_USE_LEAVE_CHANNEL_EVENT = 2;

	/**
	 * Value for the HANDLE_HOST_INIT_CHANNEL_EVENT case observer notification
	 * handler.
	 */
	private static final int HANDLE_HOST_INIT_CHANNEL_EVENT = 3;

	/**
	 * Value for the HANDLE_HOST_START_CHANNEL_EVENT case observer notification
	 * handler.
	 */
	private static final int HANDLE_HOST_START_CHANNEL_EVENT = 4;

	/**
	 * Value for the HANDLE_HOST_STOP_CHANNEL_EVENT case observer notification
	 * handler.
	 */
	private static final int HANDLE_HOST_STOP_CHANNEL_EVENT = 5;

	/**
	 * Value for the HANDLE_OUTBOUND_CHANGED_EVENT case observer notification
	 * handler.
	 */
	private static final int HANDLE_OUTBOUND_CHANGED_EVENT = 6;

	/**
	 * Enumeration of the states of the AllJoyn bus attachment. This lets us
	 * make a note to ourselves regarding where we are in the process of
	 * preparing and tearing down the fundamental connection to the AllJoyn bus.
	 * 
	 * This should really be a more private think, but for the sample we want to
	 * show the user the states we are running through. Because we are really
	 * making a data hiding exception, and because we trust ourselves, we don't
	 * go to any effort to prevent the UI from changing our state out from under
	 * us.
	 * 
	 * There are separate variables describing the states of the client ("use")
	 * and service ("host") pieces.
	 */
	public static enum BusAttachmentState {
		...
		DISCOVERING
		/**
		 * The bus attachment is discovering remote attachments hosting chat
		 * channels
		 */
	}
	 ...
	 * note to ourselves regarding where we are in the process of preparing and
	 * tearing down the AllJoyn pieces responsible for providing the chat
	...
	public static enum HostChannelState {
		...
		NAMED, /**
		 * The well-known name for the channel has been successfully
		 * acquired
		 */
		BOUND, /** A session port has been bound for the channel */
		ADVERTISED, /**
		 * The bus attachment has advertised itself as hosting an
		 * chat channel
		 */
		CONNECTED
	...
	/**
	 * The state of the AllJoyn components responsible for hosting an chat
	 * channel.
	 */
	 ...
	 * other Activities. We cannot block this thread while waiting for a network
	 * to respond, so we need to run our calls in the context of a background
	 * thread. This is the class that provides that background thread
	 * implementation.
	 * 
	 ...
	 * handler. In order to make life easier for callers, we provide API methods
	 * to deal with the actual message passing, and then when the handler thread
	 * is executing the desired method, it calls out to an implementation in the
	 * enclosing class. For example, in order to perform a connect() operation
	 * in the background, the enclosing class calls BackgroundHandler.connect();
	 * and the result is that the enclosing class method doConnect() will be
	 * called in the context of the background thread.
	 */
	private final class BackgroundHandler extends Handler {
		...
		/**
		 * Connect the application to the Alljoyn bus attachment. We expect this
		 * method to be called in the context of the main Service thread. All
		 * this method does is to dispatch a corresponding method in the context
		 * of the service worker thread.
		 */
		public void connect() {
		...
		/**
		 * Disonnect the application from the Alljoyn bus attachment. We expect
		...
		/**
		 * Start discovering remote instances of the application. We expect this
		 * method to be called in the context of the main Service thread. All
		 * this method does is to dispatch a corresponding method in the context
		 * of the service worker thread.
		 */
		...
		/**
		 * Stop discovering remote instances of the application. We expect this
		 * method to be called in the context of the main Service thread. All
		 * this method does is to dispatch a corresponding method in the context
		 * of the service worker thread.
		 */
	...
	/**
	 * The instance of the AllJoyn background thread handler. It is created when
	 * Android decides the Service is needed and is called from the onCreate()
	 * method. When Android decides our Service is no longer needed, it will
	 * call onDestroy(), which spins down the thread.
	 */
	 ...
	 * this obejct.
	 */
	private BusAttachment mBus = new BusAttachment(
			ChatApplication.PACKAGE_NAME, BusAttachment.RemoteMessage.Receive);

	/**
	 * The well-known name prefix which all bus attachments hosting a channel
	 * will use. The NAME_PREFIX and the channel name are composed to give the
	 * well-known name a hosting bus attachment will request and advertise.
	 */
	private static final String NAME_PREFIX = "org.alljoyn.bus.samples.chat";

	/**
	 * The well-known session port used as the contact port for the chat
	 * service.
	 */
	...
	private class ChatBusListener extends BusListener {
		/**
		 * This method is called when AllJoyn discovers a remote attachment that
		 * is hosting an chat channel. We expect that since we only do a
		 * findAdvertisedName looking for instances of the chat well-known name
		 * prefix we will only find names that we know to be interesting. When
		 * we find a remote application that is hosting a channel, we add its
		 * channel name it to the list of available channels selectable by the
		 * user.
		 * 
		 ...
		 * remote its name from the list of available channels selectable by the
		 * user.
		 * 
		 * In the class documentation for the BusListener note that it is a
		 * requirement for this method to be multithread safe. This is
	 ...
	 * foundAdvertisedName and lostAdvertisedName notifications. Although we
	 * often use the anonymous class idiom when talking to AllJoyn, the bus
	 * listener works slightly differently and it is better to use an explicitly
	 * declared class in this case.
	 */
	private ChatBusListener mBusListener = new ChatBusListener();

	/**
	 * Implementation of the functionality related to connecting our app to the
	 * AllJoyn bus. We expect that this method will only be called in the
	 * context of the AllJoyn bus handler thread; and while we are in the
	 * DISCONNECTED state.
	 */
	private void doConnect() {
		...
		if (Status.OK != status) {
			mChatApplication.alljoynError(ChatApplication.Module.HOST,
		...
		status = mBus.connect();
		if (status != Status.OK) {
			mChatApplication.alljoynError(ChatApplication.Module.GENERAL,
	...
	/**
	 * Implementation of the functionality related to disconnecting our app from
	 * the AllJoyn bus. We expect that this method will only be called in the
	 * context of the AllJoyn bus handler thread. We expect that this method
	 * will only be called in the context of the AllJoyn bus handler thread; and
	 * while we are in the CONNECTED state.
	 */
	 ...
	 * which are hosting chat channels. We expect that this method will only be
	 * called in the context of the AllJoyn bus handler thread; and while we are
	 * in the CONNECTED state. Since this is a core bit of functionalty for the
	 * "use" side of the app, we always do this at startup.
	 */
	...
	private void doRequestName() {
		...
		/*
		 * We depend on the user interface and model to work together to not get
		 * this process started until a valid name is set in the channel name.
		 */
		String wellKnownName = NAME_PREFIX + "."
				+ mChatApplication.hostGetChannelName();
	...
	private void doReleaseName() {
		...
		/*
		 * We need to progress monotonically down the hosted channel states for
		 * sanity.
		 */
	...
	/**
	 * Implementation of the functionality related to binding a session port to
	...
	private void doBindSession() {
		...
		Status status = mBus.bindSessionPort(contactPort, sessionOpts,
				new SessionPortListener() {
					/**
					 * This method is called when a client tries to join the
					 * session we have bound. It asks us if we want to accept
					 * the client into our session.
					 * 
					 * In the class documentation for the SessionPortListener
					 * note that it is a requirement for this method to be
					 * multithread safe. Since we never access any shared state,
					...
					/**
					 * If we return true in acceptSessionJoiner, we admit a new
					 * client into our session. The session does not really
					 * exist until a client joins, at which time the session is
					 * created and a session ID is assigned. This method
					 * communicates to us that this event has happened, and
					 * provides the new session ID for us to use.
					 * 
					 * In the class documentation for the SessionPortListener
					 * note that it is a requirement for this method to be
					 * multithread safe. Since we never access any shared state,
					 * this requirement is met.
					 * 
					 * See comments in joinSession for why the hosted chat
					...
					public void sessionJoined(short sessionPort, int id,
						...
						SignalEmitter emitter = new SignalEmitter(mChatService,
								id, SignalEmitter.GlobalBroadcast.Off);
						mHostChatInterface = emitter
	...
	/**
	 * A flag indicating that the application has joined a chat channel that it
	 * is hosting. See the long comment in doJoinSession() for a description of
	 * this rather non-intuitively complicated case.
	 */
	boolean mJoinedToSelf = false;

	/**
	 * This is the interface over which the chat messages will be sent in the
	 * case where the application is joined to a chat channel hosted by the
	 * application. See the long comment in doJoinSession() for a description of
	 * this rather non-intuitively complicated case.
	 */
	...
	private void doAdvertise() {
		...
		String wellKnownName = NAME_PREFIX + "."
				+ mChatApplication.hostGetChannelName();
		Status status = mBus.advertiseName(wellKnownName,
	...
	private void doCancelAdvertise() {
		...
		String wellKnownName = NAME_PREFIX + "."
				+ mChatApplication.hostGetChannelName();
		Status status = mBus.cancelAdvertiseName(wellKnownName,
				SessionOpts.TRANSPORT_ANY);

		if (status != Status.OK) {
			mChatApplication.alljoynError(ChatApplication.Module.HOST,
					"Unable to cancel advertisement of well-known name: ("
							+ status + ")");
			return;
		}
	...
	/**
	 * Implementation of the functionality related to joining an existing local
	 * or remote session.
	 */
	private void doJoinSession() {
		 ...
		 * joinSession() on a session port we have created with
		 * bindSessionPort() will result in an error.
		 * 
		 ...
		 * the bound chat channel. If we do not do this, the chat messages from
		 * the chat channel hosted by the application will appear in the chat
		 * channel joined by the application.
		 * 
		 ...
		 * the hosted session. There is another complexity here since the hosted
		 * session doesn't exist until a remote session has joined. This means
		 * that we don't have a session ID to use to create a SignalEmitter
		 * until a remote device does a joinSession on our hosted session. This,
		 * in turn, means that we have to create the SignalEmitter after we get
		 * a sessionJoined() callback in the SessionPortListener passed into
		 * bindSessionPort(). We chose to create the signal emitter for this
		 * case in the sessionJoined() callback itself. Note that this hosted
		 * channel signal emitter must be distinct from one constructed for the
		 * usual joinSession since a hosted channel may have a remote device do
		 * a join at any time, even when we are joined to another session. If
		 * they were not separated, a remote join on the hosted session could
		 * redirect messages from the joined session unexpectedly.
		 * 
		 ...
		 * *enables* the creation of a session. As soon as a remote device joins
		 * the hosted chat channel, a session is actually created, and the
		 * SessionPortListener sessionJoined() callback is fired. At that point,
		 * we create a separate SignalEmitter using the hosted session's
		 * sessionId that we can use to send chat messages to the channel we are
		 * hosting. As soon as the session comes up, we begin receiving chat
		 * messages from the session, so we need to filter them until the user
		 * joins the hosted chat channel. In a separate timeline, the user can
		 * decide to join the chat channel she is hosting. She can do so either
		 * before or after the corresponding session has been created as a
		 * result of a remote device joining the hosted session. If she joins
		 * the hosted channel before the underlying session is created, her chat
		 * messages will be discarded. If she does so after the underlying
		 * session is created, there will be a session emitter waiting to use to
		 * send chat messages. In either case, the signal filter will be turned
		 * off in order to listen to remote chat messages.
		 */
		if (mHostChannelState != HostChannelState.IDLE) {
			if (mChatApplication.useGetChannelName().equals(
					mChatApplication.hostGetChannelName())) {
		 ...
		 */
		String wellKnownName = NAME_PREFIX + "."
				+ mChatApplication.useGetChannelName();
		...
		Status status = mBus.joinSession(wellKnownName, contactPort, sessionId,
				sessionOpts, new SessionListener() {
					/**
					 * This method is called when the last remote participant in
					 * the chat session leaves for some reason and we no longer
					 * have anyone to chat with.
					 * 
					 * In the class documentation for the BusListener note that
					 * it is a requirement for this method to be multithread
					 * safe. This is accomplished by the use of a monitor on the
					 * ChatApplication as exemplified by the synchronized
					 * attribute of the removeFoundChannel method there.
					 */
					@Override
					public void sessionLost(int sessionId) {
						Log.i(TAG, "BusListener.sessionLost(" + sessionId + ")");
						mChatApplication.alljoynError(
								ChatApplication.Module.USE,
								"The chat session has been lost");
		...
		if (status == Status.OK) {
			...
			mChatApplication.alljoynError(ChatApplication.Module.USE,
					"Unable to join chat session: (" + status + ")");
			return;
		}

		SignalEmitter emitter = new SignalEmitter(mChatService, mUseSessionId,
	...
	/**
	 * Implementation of the functionality related to joining an existing remote
	 * session.
	 */
	...
	/**
	 * The session identifier of the "use" session that the application uses to
	 * talk to remote instances. Set to -1 if not connectecd.
	 */
	int mUseSessionId = -1;

	/**
	 * Implementation of the functionality related to sending messages out over
	 * an existing remote session. Note that we always send all of the messages
	 * on the outbound queue, so there may be instances where this method is
	 * called and we find nothing to send depending on the races.
	 */
	private void doSendMessages() {
		...
		while ((message = mChatApplication.getOutboundItem()) != null) {
			 ...
			 * the mChatInterface. If we are implicityly joined to a session we
			 * are hosting, we send the message over the mHostChatInterface. The
			 * mHostChatInterface may or may not exist since it is created when
			 * the sessionJoined() callback is fired in the SessionPortListener,
	...
	class ChatService implements ChatInterface, BusObject {
		/**
		 * Intentionally empty implementation of Chat method. Since this method
		 * is only used as a signal emitter, it will never be called directly.
		 */
		@Override
