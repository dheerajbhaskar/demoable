...
public class Service extends Activity {
	...
	public void onResume() {
		...
		/*
		 * The auto-accept handler is automatically deregistered when the
		 * application goes in to the background, so it must be registered again
		 * here in onResume().
		 * 
		 * Since any push-button group formation request will be accepted while
		 * the auto-accept object is intercepting requests, only call
		 * intercept(true) when the application is expecting incoming
		 * connections. Call intercept(false) as soon as incoming connections
		 * are not expected.
		 */
	...
	protected void onStop() {
		...
		/*
		 * While the auto-accept handler can automatically de-register when the
		 * app goes in to the background or stops, it's a good idea to
		 * explicitly de-register here so the handler is in a known state if the
